...
select requestid                                                            as request_id,
       ...
       min(
                   extract(epoch from (arrivadate - departuredate))::float
...
from client_requests
         join client_request_parsed_info pi on client_requests.id = pi.client_request_id
...
select client_requests.id,
       ...
       extract(epoch from (arrivadate - departuredate)) - (to_city.timezone - from_city.timezone) *
                                                           3600                                           as to_time_abs,
